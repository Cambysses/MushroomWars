...
public class Grid<TGridObject>
{
    ...
    public Grid(int width, int height, float cellSize, Vector3 originPosition, Func<Grid<TGridObject>, int, int, TGridObject> createGridObject)
    {
        ...
        if (isDebug)
        {
            for (int x = 0; x < gridArray.GetLength(0); x++)
            {
                for (int y = 0; y < gridArray.GetLength(1); y++)
                {
                    debugTextArray[x, y] = UtilsClass.CreateWorldText(gridArray[x, y]?.ToString(), null, GetWorldPosition(x, y) + new Vector3(0.5f, 0.5f), 36, 0.1f, Color.white, TextAnchor.MiddleCenter);
    ...
    {
        return new Vector3(x + offset, y + offset) * cellSize + originPosition;
    }

    public void GetXYFromPosition(Vector3 worldPosition, out int x, out int y)
    {
    ...
    public void SetGridObject(Vector3 worldPoisition, TGridObject value)
    {
        int x, y;
        GetXYFromPosition(worldPoisition, out x, out y);
    ...
    private void DrawPath(List<PathNode> path)
    {
        if (path != null)
        {
            ...
            {
                Vector3 worldPoisitionStart = this.GetWorldPosition(path[i].x, path[i].y);
                Vector3 worldPoisitionEnd = this.GetWorldPosition(path[i + 1].x, path[i + 1].y);
